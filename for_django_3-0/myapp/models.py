...
def YuLab_fractionxchannel_IP_DIA(job_id, prot_init_df, raw_input_df, annotation_df, bait_name, comparison, comp_type, row_norm_user,column_norm_user, bait_norm, num_channels, norm_prots, pval_calc, uniprotGene, imputation, fusion_aa_seq='', iso_filter=100):
    ...
    channel_list = list(set(annotation_df.loc[(annotation_df["Label"]!="none")&((annotation_df["Label"]==wt)|(annotation_df["Label"]==ctrl))]["Channel"].values.tolist()))
    channel_list = sort1(channel_list)
    
    # Will run row normalization by default
    prot_init_df = preprocess_byrownorm(prot_init_df,False,channel_list)#row_norm_user
    ...
    # channel_wt = annotation_df.loc[annotation_df["Label"]==wt]["Channel"].values.tolist()
    # wt_filter_vals = {}
    # for channel in channel_wt:
    #     if percentage_thresh>0:
    #         tmp_df = prot_init_df[["log2_"+channel+"_1"]].dropna()
    #         vals = sorted(tmp_df["log2_"+channel+"_1"].values.tolist(), key = lambda x: x, reverse = False)
    #         N = int(percentage_thresh*len(vals))
    #         res = vals[:N]
    #         print("log2_"+channel+"_1", res[-1], N)
    #         wt_filter_vals["log2_"+channel+"_1"] = res[-1]
    # print(wt_filter_vals)
    # # Calculate number of missing values per row (if <3, remove all values in row)
    # if percentage_thresh>0:
    #     for i in range(prot_init_df.shape[0]):
    #         num_channels = 0
    #         gene = prot_init_df.iloc[i]["Genes"]
    #         for channel in channel_wt:
    #             if prot_init_df.iloc[i]["log2_"+channel+"_1"] < wt_filter_vals["log2_"+channel+"_1"]:
    #                 num_channels+=1
    #         if num_channels > 1:
    #             for channel in channel_wt:
    #                 prot_init_df.loc[prot_init_df["Genes"]==gene,"log2_"+channel+"_1"] = np.nan
...
def YuLab_fractionxchannel_DIA(job_id, init_df, raw_input_df, annotation_df, bait_name, comparison, comp_type, row_norm_user,column_norm_user, bait_norm, num_channels, norm_prots, pval_calc, uniprotGene, fusion_aa_seq='', imputation_user=False, iso_filter=100):
    ...
    print(list(init_df))
    init_df = preprocess_byrownorm(init_df,False,channel_list)#row_norm_user
...
def SILAC_COMETconversion(sn_df, annotation_df, file_type):
    ## Columns needed libra -> Abundance columns
    ## Fraction/ Spectrum File -> spectrum
    ## Intensity -> precursor_intensity
    ## Annotated Sequence -> peptide but diff
    ## Sequence needs to be made
    ## charge -> assumed_charge
    ## Master protein accession -> protein (only using unique)
    ## # Protein Groups -> num_prots
    ## RT [min] -> retention_time_sec/60
    
    file_types = list(set(annotation_df["File"].values.tolist()))
    ...
    # sn_df["Abundance: WT_"+file_type] = np.nan
    # sn_df["Abundance: CNTRL_"+file_type] = np.nan
    
    tmpdf = annotation_df.loc[annotation_df["File"]==file_type]
    channels = list(set(tmpdf["Channel"].values.tolist()))
    for channel in channels:
        label = tmpdf.loc[tmpdf["Channel"]==channel]["Label"].values.tolist()[0]
        sn_df["Abundance: "+ label +"_"+file_type] = np.nan
        if (channel=="Heavy") | (channel=="heavy"):
            sn_df["Abundance: "+ label +"_"+file_type] = sn_df["heavy_area"]
        elif (channel=="Light") | (channel=="light"):
            sn_df["Abundance: "+ label +"_"+file_type] = sn_df["light_area"]
        else:
            sn_df["Abundance: "+ label +"_"+file_type] = sn_df["medium_area"]
    ...
    sn_df[cols_abundance] = sn_df[cols_abundance].replace({'-':np.nan})#, 0:np.nan})

    # Using all except ambiguous ID's (this is specifically for benchmark datasets)
    sn_df = sn_df.loc[~(sn_df["protein"].str.contains("HUMAN")&sn_df["protein"].str.contains("YEAST"))]
    sn_df["# Protein Groups"] = 1
    sn_df["PeptideIDMatch"] = sn_df["Annotated Sequence"] + "_@_" + sn_df["Charge"].astype(str) + "_@_" + sn_df["Master Protein Accessions"]
    return sn_df

def YuLab_SILAC(job_id, fwd_rep, rev_rep, annotation_df, bait_name, comparison, comp_type, row_norm_user, column_norm_user, norm_prots, uniprotGene):
    # fwd_rep = changeCOMEToutput(fwd_rep,annotation_df,"FWD")
    # rev_rep = changeCOMEToutput(rev_rep,annotation_df,"REV")
    annotation_df["TypeChannel"] = annotation_df["Label"]+"_"+annotation_df["File"]
    channel_list = list(set(annotation_df.loc[annotation_df["Label"]!="none"]["TypeChannel"].values.tolist()))
    channel_list = sort1(channel_list)
    wt = ""
    ctrl = ""
    if comp_type==1:
    ...
    final_df = []
    final_df = fwd_rep
    # Add the Abundance columns from rev rep based on Annotated Sequence and charge and protein
    file_type="REV"
    final_df["Abundance: "+ctrl+"_"+file_type] = final_df.PeptideIDMatch.map(rev_rep.set_index("PeptideIDMatch")["Abundance: "+ctrl+"_"+file_type].to_dict())
    final_df["Abundance: "+wt+"_"+file_type] = final_df.PeptideIDMatch.map(rev_rep.set_index("PeptideIDMatch")["Abundance: "+wt+"_"+file_type].to_dict())

    # final_df["Spectrum File REV"] = final_df.PeptideIDMatch.map(rev_rep.set_index("PeptideIDMatch")["Spectrum File"].to_dict())
    final_df["Spectrum File"] = "f1.raw"
    # Row normalization = False (default)
    peak_df = psm_summarization_SILAC(final_df, True, annotation_df, channel_list)
    ...
    # NO Bait-based normalization for SILAC (because with two/ three channels no condition comparison that would require this)
    prot_df = expansion_to_fractionXchannel_SILAC(peak_df,channel_list)
    final_prot_df = calc_FC_and_pval_SILAC(job_id, prot_df, final_df, comparison, comp_type, uniprotGene)
    # final_prot_df["Label"] = comparison
    
    psm_cutoff = 5
    if bait_name != "NA":
        psm_cutoff = psm_cutoff_suggestion(bait_name, final_prot_df)
    final_prot_df.at[0,"PSM Cutoff"] = psm_cutoff
    print("Done calculating FC and p-value for: ", comparison)
    ...
    # peak_df_deliverable = cleanup_peak_df_SILAC(peak_df, annotation_df, comparison, comp_type, channel_list)
    # print("Cleaned peptide deliverable excel sheet")

    return (final_prot_df, peak_df)

def psm_summarization_SILAC(psm_df, row_norm_user, annotation_df, channel_list):
    # Filtered PSMs using non-shared PSMs, 
    cols_abund = []
    for channel in channel_list:
        cols_abund.append("Abundance: "+channel)
        psm_df["Abundance: "+channel] = pd.to_numeric(psm_df["Abundance: "+channel] )
    ...
    psm_df.loc[:,"PSMID"] = psm_copy["Annotated Sequence"] + "_@_" + psm_copy["Charge"].astype(str) + "_@_"+ psm_copy["Master Protein Accessions"]#.str.split("_").str[-1]
...
def expansion_to_fractionXchannel_SILAC(peak_df, channel_list):
    #peak_df[["AnnotatedSeq","Charge","Fraction"]] = peak_df["PeakID"].str.split("_@_",expand=True)
    # Do the same as above but for all peptides belonging to proteins
    ...
    for i in range(len(prot_list)):
        prot = prot_list[i]
        df = peak_df.loc[(peak_df["Protein"]==prot)]
        for channel in channel_list:
            if len(df["log2_"+str(channel)+"_1"].dropna().values.tolist())>0:
                dict_list.append({"Protein":prot,"log2Abundance":statistics.median(df["log2_"+str(channel)+"_1"].dropna().values.tolist()),"Channel":str(channel),"Condition":str(channel),"Fraction":"f1.raw"})
...
def create_limma_suitable_file_SILAC(wt, ctrl, prot_df, prot_list):
    # Assuming three replicates present
    num_reps = 2#len(list(set(prot_df.loc[prot_df["Condition"]==wt]["Channel"].values.tolist())))
    # ctrl_channels = list(set(prot_df.loc[prot_df["Condition"]==ctrl]["Channel"].values.tolist()))
    cols_rep = []
    for i in range(num_reps):
        cols_rep+=("rep"+str(i+1))
    ...
    for prot in prot_list:
        ...
        wt_fwd = prot_df.loc[(prot_df["Protein"]==prot)&(prot_df["Condition"]==wt+"_FWD")]["log2Abundance"].values.tolist()[0]
        cntrl_fwd = prot_df.loc[(prot_df["Protein"]==prot)&(prot_df["Condition"]==ctrl+"_FWD")]["log2Abundance"].values.tolist()[0]
        wt_rev = prot_df.loc[(prot_df["Protein"]==prot)&(prot_df["Condition"]==wt+"_REV")]["log2Abundance"].values.tolist()[0]
        cntrl_rev = prot_df.loc[(prot_df["Protein"]==prot)&(prot_df["Condition"]==ctrl+"_REV")]["log2Abundance"].values.tolist()[0]
        
        if (wt_fwd==np.nan)|(wt_rev==np.nan)|(cntrl_fwd==np.nan)|(cntrl_rev==np.nan):
            continue
        else:
            df2 = {"Protein":prot,"rep1":(wt_fwd-cntrl_fwd),"rep2":(wt_rev-cntrl_rev)}
            shallow_copy = {}
            for k,val in df2.items():
                shallow_copy[k] = val
            dict_list.append(shallow_copy)
...
def calc_pval_withratios_SILAC(job_id, wt, ctrl, prot_df, prot_list, prot_df_calc):
    prot_df_calc["pval"] = np.nan
    limma_pre_df = []
    limma_pre_df = create_limma_suitable_file_SILAC(wt, ctrl, prot_df, prot_list)
    # print(limma_pre_df.loc[limma_pre_df["Protein"]=="P04803"])
    print("Done limma setup with SILAC")
    ...
    filename_lim = local_storage_dir+"/"+job_id+"/"+wt+"-"+ctrl+"_SILAC"
...
def calc_FC_and_pval_SILAC(job_id, prot_df, init_psm_df, comparison, comp_type, uniprotGene):
    prot_list = list(set(prot_df["Protein"].dropna().values.tolist()))
    ...
    for prot in prot_list:
        ...
        all_log2fc = [(wt_fwd-cntrl_fwd),(wt_rev-cntrl_rev)]
    ...
    # Limma processing   
    prot_median_manual = calc_pval_withratios_SILAC(job_id, wt, ctrl, prot_df, prot_list, prot_median_manual)

    prot_median_manual["log2FC"] = np.log2(prot_median_manual["FC"])
    print("Initial # proteins: ",prot_median_manual.shape[0])
    prot_median_manual_dropna = prot_median_manual[["Protein","log2FC","pval"]].dropna()
    print("After which have p-values filter # proteins: ",prot_median_manual_dropna.shape[0])
...
def YuLab_phosphopep_fractionxchannel(job_id, sn_df, intensity_df, annotation_df, bait_name, comparison, comp_type, fraction_use, row_norm_user,column_norm_user, bait_norm, num_channels, norm_prots, pval_calc, uniprotGene, fusion_aa_seq='', imputation_user=False, iso_filter=100):
    ...
    print("Calculating peptide level FC, FDR estimates")
    peak_df[["AnnotatedSeq","Charge","Fraction"]] = peak_df["PeakID"].str.split("_@_",expand=True)
    peak_df["UniquePeptide"] = peak_df["AnnotatedSeq"] + "_@_" + peak_df["Charge"].astype(str) + "_@_" + peak_df["Protein"].astype(str)
    # peak_df_deliverable = cleanup_peak_df_phosphopep(peak_df, annotation_df, comparison, comp_type, channel_list, pval_calc)
    peak_df_deliverable = cleanup_peak_df_phosphopep_NEW(peak_df, annotation_df, comparison, comp_type, channel_list, pval_calc, job_id)

    peak_df_deliverable[["AnnotatedSeq","Charge","Protein"]] = peak_df_deliverable["UniquePeptide"].str.split("_@_",expand=True)
    ...
    if isinstance(init_psm_df, pd.DataFrame):
        forpsm_df = init_psm_df
        # pep_list = list(set(peak_df_deliverable["UniquePeptide"].dropna().values.tolist()))
        prot_list = list(set(peak_df_deliverable["Protein"].dropna().values.tolist()))
        peak_df_deliverable["# PSMs"] = 0
        for p in prot_list:
            # fil = forpsm_df.loc[(forpsm_df["Annotated Sequence"]==p.split("_")[0])&(forpsm_df["Charge"]==int(p.split("_")[1]))]
            fil = forpsm_df.loc[(forpsm_df["Master Protein Accessions"].str.contains(p, na=False))]
...
def calc_FC_withratios_pep(wt_channels, ctrl_channels, peak_df, pep_list, peak_df_calc):
    for pep in pep_list:
        fraction_list = list(set(peak_df.loc[(peak_df["UniquePeptide"]==pep)]["Fraction"].values.tolist()))
        per_fraction_FC = []
        for frac in fraction_list:
            # Calculate FC
            rvs_T = []
            if (len(peak_df.loc[(peak_df["UniquePeptide"]==pep)&(peak_df["Fraction"]==frac)][wt_channels].dropna().values.tolist())>0):
                # sum below flattens list of lists
                rvs_T = sum(peak_df.loc[(peak_df["UniquePeptide"]==pep)&(peak_df["Fraction"]==frac)][wt_channels].dropna().values.tolist(),[])
            rvs_C = []
            if (len(peak_df.loc[(peak_df["UniquePeptide"]==pep)&(peak_df["Fraction"]==frac)][ctrl_channels].dropna().values.tolist())>0):
                # sum below flattens list of lists
                rvs_C = sum(peak_df.loc[(peak_df["UniquePeptide"]==pep)&(peak_df["Fraction"]==frac)][ctrl_channels].dropna().values.tolist(),[])
            
            for t in rvs_T:
                for c in rvs_C:
                    per_fraction_FC.append(np.power(2,t-c))
        peak_df_calc.loc[peak_df_calc["UniquePeptide"]==pep,"FC"] = np.nanmedian(per_fraction_FC)
    
    return peak_df_calc#_dropna

def create_limma_suitable_file_pep(wt_channels, ctrl_channels, peak_df, pep_list, peak_df_calc):
    num_reps = np.max([len(list(wt_channels)),len(list(ctrl_channels))])
    cols_rep = []
    for i in range(num_reps):
        cols_rep+=("rep"+str(i+1))
    ...
    for pep in pep_list:
        ...
        for frac in fraction_list:
            ...
            if len(rvs_T) > len(rvs_C):
                df2 = {"UniquePeptide":pep,"Fraction":frac}
                pos_C = list(permutations(id_T, len(id_C)))
                for possibility in pos_C:
                    for id_ in range(len(possibility)):
                        df2["rep"+str(id_+1)] = (rvs_T[possibility[id_]]-rvs_C[id_])
...
def calc_pval_withratios_pep(job_id, wt, ctrl, wt_channels, ctrl_channels, peak_df, pep_list, peak_df_calc):
    peak_df_calc["pval"] = np.nan
    limma_pre_df = create_limma_suitable_file_pep(wt_channels, ctrl_channels, peak_df, pep_list, peak_df_calc)
    ...
    filename_lim = local_storage_dir+"/"+wt+"-"+ctrl+"_prelimma_peptide"
    limma_pre_df.to_csv(filename_lim+".txt",sep="\t")
    limma_post_df = run_r_script(limma_pre_df,filename_lim)
    limma_summarized = limma_post_df.groupby(['UniquePeptide'])['pvalue','FDR'].mean().reset_index()
    peak_df_calc["pval"] = peak_df_calc.UniquePeptide.map(limma_summarized.set_index("UniquePeptide")["pvalue"].to_dict())
    peak_df_calc["adjpval"] = peak_df_calc.UniquePeptide.map(limma_summarized.set_index("UniquePeptide")["FDR"].to_dict())
    return peak_df_calc

def cleanup_peak_df_phosphopep_NEW(peak_df, annotation_df, comparison, comp_type, channel_list, pval_calc, job_id):
    cols_to_keep = ["UniquePeptide"]#,"PeakID","AnnotatedSeq","Charge","Fraction","Protein"]
    # # pep_list = list(set(peak_df["UniquePeptide"].values.tolist()))
    ...
    pep_det = {
        'UniquePeptide':pep_list
    }

    peak_df_calc = pd.DataFrame(pep_det)

    # TODO Calculate log2FC, p-value and adjusted p-value per row here
    # peak_df_calc["log2FC"] = np.nan
    peak_df_calc["FC"] = np.nan
    peak_df_calc["pval"] = np.nan
    ...
    # # Calculate FC
    peak_df_calc = calc_FC_withratios_pep(wt_channels, ctrl_channels, peak_df, pep_list, peak_df_calc)
    peak_df_calc["log2FC"] = np.log2(peak_df_calc["FC"])
    print("Calculated FC per peak successfully")
    # Calculate p-value using t-test
    if pval_calc == "LinearModel":
        for pep in pep_list:
            wt_vals = sum(peak_df.loc[(peak_df["UniquePeptide"]==pep)][wt_channels].dropna().values.tolist(),[])
            ctrl_vals = sum(peak_df.loc[(peak_df["UniquePeptide"]==pep)][ctrl_channels].dropna().values.tolist(),[])
            if (len(wt_vals)>1)&(len(ctrl_vals)>1):
                peak_df_calc.loc[peak_df_calc["UniquePeptide"]==pep,"pval"] = stats.ttest_ind(wt_vals, ctrl_vals).pvalue
                
    elif pval_calc == "Limma":
        peak_df_calc = calc_pval_withratios_pep(job_id, wt, ctrl, wt_channels, ctrl_channels, peak_df, pep_list, peak_df_calc)
    else:
        print("Error: Magma Type not chosen")
    print("Calculated pval per peak successfully")
    cols_to_keep += ["log2FC","FC","pval"]
    peak_df_calc = peak_df_calc[cols_to_keep].dropna()
    peak_df_calc["adjpval"] = statsmodels.stats.multitest.multipletests(peak_df_calc["pval"].values.tolist(),method="fdr_bh")[1]

    cols_to_keep += ["adjpval"]

    for col in channel_list:
        peak_df_calc[str(col)] = np.nan
        for pep in pep_list:
            peak_df_calc.loc[peak_df_calc["UniquePeptide"]==pep,str(col)] = np.power(2,np.nanmedian(peak_df.loc[peak_df["UniquePeptide"]==pep]["log2_"+col+"_1"].values.tolist()))
        cols_to_keep.append(str(col))
    peak_df_calc = peak_df_calc[cols_to_keep]
    return peak_df_calc

def cleanup_peak_df_phosphopep(peak_df, annotation_df, comparison, comp_type, channel_list, pval_calc):
...
#######################################################################################
#######################################################################################
############# Regular functions used by all functionalities ###########################
#######################################################################################
#######################################################################################
...
def psm_summarization(sn_df, intensity_df, annotation_df, row_norm_user, channel_list, iso_filter=100):
    ...
    #Check if fraction separation is problem
    # psm_df["PSMID"] = psm_df["Annotated Sequence"] + "_" + psm_df["Charge"].astype(str) + "_1.raw"
    psm_df.loc[:,"PSMID"] = psm_copy["Annotated Sequence"] + "_@_" + psm_copy["Charge"].astype(str) + "_@_"+ psm_copy["Spectrum File"]#.str.split("_").str[-1]
...
# TODO - Remove the adjusted pvalue threshold to 0.5 raw if nothing passes FDR
