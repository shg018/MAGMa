...
def SILAC_COMETconversion(sn_df, annotation_df, file_type):
    ...
    for channel in channels:
        ...
        if (channel=="Heavy") | (channel=="heavy"):
            sn_df["Abundance: "+ channel +"_"+file_type] = sn_df["heavy_area"]
        elif (channel=="Light") | (channel=="light"):
            sn_df["Abundance: "+ channel +"_"+file_type] = sn_df["light_area"]
        else:
            sn_df["Abundance: "+ channel +"_"+file_type] = sn_df["medium_area"]
    ...
    sn_df["PeptideIDMatch"] = sn_df["Annotated Sequence"] + "_" + sn_df["Charge"].astype(str) + "_" + sn_df["Master Protein Accessions"]
    return sn_df
...
def calc_FC_and_pval_SILAC(prot_df, init_psm_df, comparison, comp_type, uniprotGene):
    prot_list = list(set(prot_df["Protein"].dropna().values.tolist()))
...
def YuLab_phosphopep_fractionxchannel(job_id, sn_df, intensity_df, annotation_df, bait_name, comparison, comp_type, fraction_use, row_norm_user,column_norm_user, bait_norm, num_channels, norm_prots, pval_calc, uniprotGene, fusion_aa_seq='', imputation_user=False, iso_filter=100):
    ...
    peak_df["UniquePeptide"] = peak_df["AnnotatedSeq"] + "_" + peak_df["Charge"].astype(str) + "_" + peak_df["Protein"].astype(str)
    ...
    peak_df_deliverable[["AnnotatedSeq","Charge","Protein"]] = peak_df_deliverable["UniquePeptide"].str.split("_",expand=True)
